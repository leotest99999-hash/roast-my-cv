...
export const roastAnalysisSchema = z.object({
  ...
  wins: z.array(z.string().min(12).max(2000)).min(2).max(4),
  ...
  upgradePitch: z.object({
    ...
    points: z.array(z.string().min(12).max(2000)).min(3).max(4),
  }),
...
export const rewriteResultSchema = z.object({
  ...
  improvements: z.array(z.string().min(12).max(2000)).min(3).max(5),
